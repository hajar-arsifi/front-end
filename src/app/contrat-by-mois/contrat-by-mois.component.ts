...
@Component({
  selector: 'app-contrat-by-mois',
  templateUrl: './contrat-by-mois.component.html',
  styleUrls: ['./contrat-by-mois.component.css']
})
export class ContratByMoisComponent implements OnInit {
  ...
  constructor(private services: ContratsService, private router: Router, private activatedRoute: ActivatedRoute,
              private modalService: ModalManager, private projetService: GestionProjetService,) { }

  ngOnInit(): void {
    const annee = this.activatedRoute.snapshot.params['annee'];
    const mois = this.activatedRoute.snapshot.params['mois'];
    this.services.getContratByAnneeAndMois(annee, mois, this.currentPage, this.size)
      .subscribe((data: any) => {
